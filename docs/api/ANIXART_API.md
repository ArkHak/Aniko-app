...
6. ~~`GET profile/{id}` падает с `JsonConvertException` на аккаунте с непустой историей — `history[].last_view_episode` приходит ПОЛНЫМ объектом эпизода~~.
   **Исправлено (2026-09-18):** `LastViewEpisodeSerializer` (`shared/data/.../dto/LastViewEpisodeSerializer.kt`) принимает обе формы: число и объект эпизода; из объектной формы (`{"@id":...,"releaseId":...,"position":...,"release":{...}}`, встречается в `history[]` и списочных эндпоинтах `profile/list`, `favorite`, `discover/watching`) достаёт `position` — это и есть номер последней просмотренной серии (его же принимает `EpisodeApi.watch`). Благодаря этому «Мои списки»/история показывают реальное «N из M» вместо вечного «0 из M».
