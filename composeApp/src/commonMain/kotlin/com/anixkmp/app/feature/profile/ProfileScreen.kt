package com.anixkmp.app.feature.profile
...
import com.anixkmp.ui.component.AnixAvatar
...
fun ProfileScreen(
    ...
    Scaffold(
        ...
        when {
            uiState.isLoading && profile == null -> AnixLoadingBox(
                modifier = Modifier.fillMaxSize().padding(innerPadding),
            )

            uiState.error != null && profile == null -> AnixErrorBox(
                message = uiState.error?.message ?: "Не удалось загрузить профиль",
                onRetry = viewModel::retry,
                modifier = Modifier.fillMaxSize().padding(innerPadding),
            )

            profile != null && privacy != null -> ProfileContent(
                profile = profile,
                privacy = privacy,
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
private fun ProfileHeader(profile: ProfileDetails, modifier: Modifier = Modifier) {
    val dimens = AnixThemeTokens.dimens
...
/** Простая сетка статистики: по 4 подписанных числа в ряд — оправданно проще `LazyVerticalGrid` для 8 фиксированных полей без скролла. */
@Composable
private fun StatsGrid(profile: ProfileDetails, modifier: Modifier = Modifier) {
    val dimens = AnixThemeTokens.dimens
    val stats = listOf(
...
@Composable
private fun PrivacyRow(title: String, selected: PrivacyVisibility, onSelect: (PrivacyVisibility) -> Unit) {
    val dimens = AnixThemeTokens.dimens
...
@Composable
private fun FriendRequestPrivacyRow(selected: FriendRequestVisibility, onSelect: (FriendRequestVisibility) -> Unit) {
    val dimens = AnixThemeTokens.dimens
    Column(verticalArrangement = Arrangement.spacedBy(dimens.spaceXs)) {
...
private fun PrivacyVisibility.toDisplayName(): String = when (this) {
    PrivacyVisibility.EVERYONE -> "Все"
    PrivacyVisibility.FRIENDS_ONLY -> "Друзья"
    PrivacyVisibility.ONLY_ME -> "Только я"
}

private fun FriendRequestVisibility.toDisplayName(): String = when (this) {
