...
откройте и перетащите `Aniko.app` в `Applications`. Приложение не подписано и не нотаризовано Apple —
при первом запуске разрешите его через Настройки → Конфиденциальность и безопасность → «Открыть в
любом случае» (или соберите `.dmg` из исходников, см. ниже).
