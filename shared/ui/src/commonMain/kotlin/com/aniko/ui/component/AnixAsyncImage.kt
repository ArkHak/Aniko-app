 ...
 *
 * Пока картинка грузится и при ошибке показывает статичную PNG-иллюстрацию (при загрузке — с
 * лёгкой пульсацией альфы, при ошибке — полностью статично); после успеха рендерит изображение
 * так же, как обычный `AsyncImage` (crossfade берётся из синглтон-конфигурации
 * [coil3.SingletonImageLoader], см. `App.kt`).
 *
