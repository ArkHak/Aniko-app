...
sealed class AnixError(
    ...
    class Network(
        cause: Throwable? = null,
    ) : AnixError("Нет соединения с сервером", cause)
    ...
    class Unauthorized(
        cause: Throwable? = null,
    ) : AnixError("Требуется вход в аккаунт", cause)

    /** Не удалось разобрать ответ. */
    class Parsing(
        cause: Throwable? = null,
    ) : AnixError("Не удалось разобрать ответ сервера", cause)
    ...
    class Unknown(
        cause: Throwable? = null,
    ) : AnixError("Неизвестная ошибка", cause)
}
