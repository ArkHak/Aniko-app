...
data class ProfileDetailsDto(
    ...
    /**
     * P16.T13 (профиль-витрина, «обложка»): живая проверка `GET profile/{id}` (2026-09-10,
     * несколько реальных id без токена) подтвердила поле `theme_background_url` в полном ответе —
     * URL картинки-фона витрины пользователя (`null` у всех проверенных аккаунтов в этой пробе,
     * похоже заполняется только у тех, кто настроил тему витрины). Отдельной серверной
     * "обложки"-баннера в ответе нет — это оно и есть, ближайший реальный аналог. Соседние
     * `theme_gradient_*`/`theme_icon_*` (тоже реальные, тоже почти всегда `null`) сюда не
     * заведены — это отдельная, недокументированная механика платной кастомизации темы (не
     * genre-based авто-тема), вне объёма этой задачи (см. KDoc [com.aniko.model.ProfileDetails.coverUrl]).
     */
    @SerialName("theme_background_url") val themeBackgroundUrl: String? = null,
)
